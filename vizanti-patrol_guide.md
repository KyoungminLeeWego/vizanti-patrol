...
10. [커스텀 위젯 — Patrol Route](#10-커스텀-위젯--patrol-route)
    - [웨이포인트 이름 및 순찰 스크립트](#웨이포인트-이름-및-순찰-스크립트)
11. [커스텀 위젯 — Patrol History](#11-커스텀-위젯--patrol-history)
12. [문제 해결](#12-문제-해결)
...
| **📋 경로명** | 현재 편집 중인 경로의 이름. 불러온 경로는 자동 표시, 직접 입력도 가능 |
...
| **이름 입력** (각 웨이포인트) | 웨이포인트에 사람이 읽기 쉬운 이름 부여 (예: `정문`, `창고A`) |
| **순찰 스크립트** (각 웨이포인트) | 로봇이 해당 지점 도착 시 실행할 기능 선택 (아래 표 참고) |
...
각 웨이포인트에는 **이름**과 **순찰 스크립트**를 지정할 수 있습니다. 웨이포인트 목록의 각 항목에서 바로 설정합니다.

#### 웨이포인트 이름

`이름(선택)` 입력란에 웨이포인트의 역할이나 위치를 나타내는 이름을 입력합니다.

- 예: `정문`, `창고A`, `엘리베이터 앞`, `충전 스테이션`
- 이름은 경로 저장 시 함께 저장되며, `patrol_api`가 실행 로그와 상태에도 활용합니다.
- 이름을 지정하지 않아도 순찰에는 영향 없습니다.

#### 순찰 스크립트 (patrol_scripts)

웨이포인트 도착 시 자동으로 실행할 기능을 드롭다운에서 선택합니다. 선택된 스크립트는 `patrol_api`를 통해 백엔드로 전달됩니다.

| 스크립트 값 | 표시 | 설명 |
|------------|------|------|
| *(없음)* | 없음 | 도착 후 즉시 다음 웨이포인트로 이동 |
| `yolo_camera` | 📷 카메라 | YOLO 기반 객체 감지 카메라 촬영 수행 |
| `photo` | 🖼 사진 저장 | 현재 시점 카메라 이미지를 파일로 저장 |
| `lidar_check` | 📡 전방 장애물 감지 | LiDAR 데이터로 전방 일정 거리 내 장애물 유무 확인 |

> **참고:** 순찰 스크립트는 `patrol_api`에서 처리합니다. 지원하는 스크립트 목록 및 동작 방식은 `patrol_api` 패키지 문서를 참조하세요.

#### 순찰 스크립트 적용 예시

```
웨이포인트 1 — 이름: "정문"     스크립트: 📷 카메라
웨이포인트 2 — 이름: "복도 중앙" 스크립트: 없음
웨이포인트 3 — 이름: "창고 입구" 스크립트: 📡 전방 장애물 감지
웨이포인트 4 — 이름: "비상구"   스크립트: 📡 전방 장애물 감지
```

이렇게 설정하면 로봇이 각 지점에 도달할 때마다 지정된 기능을 순서대로 수행합니다.
...
```json
{
  ...
  "yaw": 0.0,
  "name": "정문",
  "patrol_options": ["yolo_camera"]
}
```

| 필드 | 타입 | 설명 |
|------|------|------|
| `x`, `y` | float | 맵 기준 좌표 (미터) |
| `yaw` | float | 도착 방향 (라디안) |
| `name` | string | 웨이포인트 이름 (선택) |
| `patrol_options` | string[] | 도착 시 실행할 스크립트 목록. 현재 지원: `yolo_camera`, `photo`, `lidar_check` |

---

## 11. 커스텀 위젯 — Patrol History

### 개요

순찰 이력을 조회하는 위젯입니다. 순찰이 완료될 때마다 `patrol_api`가 `~/.ros/patrol_history/`에 JSON 파일로 저장한 이력을 목록·상세 화면으로 확인하고 삭제할 수 있습니다.
...
|---------|------|
| **🔄 새로고침** | 이력 목록을 다시 불러옴 |
| **이력 목록** | 최신순으로 정렬된 순찰 이력. 상태 아이콘·시작 시각·완료 지점 수 표시 |
| **삭제** (각 항목) | 해당 이력 파일 삭제 |
...
|---------|------|
| **← 뒤로** | 목록 화면으로 돌아감 |
| **헤더** | 시작 시각, 종료 시각, 상태, 반복 여부 요약 |
| **웨이포인트 목록** | 각 지점의 도착 시각, 좌표, 순찰 스크립트 실행 결과 표시 |
...
1. Vizanti `+` 버튼 → **Patrol History** 위젯 추가
2. 패널을 열면 이력 목록이 자동으로 로드됨
3. 항목을 클릭하면 해당 순찰의 웨이포인트별 상세 결과 확인 가능
4. **삭제** 버튼으로 개별 이력 삭제, **← 뒤로**로 목록 복귀

### 백엔드 연동

| 동작 | API 호출 |
|------|----------|
| 이력 목록 조회 | `GET /api/patrol/history` |
| 이력 상세 조회 | `GET /api/patrol/history/{id}` |
| 이력 삭제 | `DELETE /api/patrol/history/{id}` |

이력 파일 저장 경로: `~/.ros/patrol_history/{YYYYMMDD_HHMMSS}.json`
